...
if (!data.directDebts) data.directDebts = [];
...
function openAddDebt(type) {
    ...
    if (type === "customer") {
        title.textContent = "➕ دين لنا (على زبون)";
        body.innerHTML = `
            <label>اختر زبون موجود (أو اكتب اسم جديد)</label>
            <input type="text" id="debt_person" list="debtPersonsList" placeholder="اسم الزبون" autocomplete="off">
            <datalist id="debtPersonsList">
                ${data.customers.map(c => `<option value="${escapeHTML(c.name)}"></option>`).join("")}
            </datalist>

            <label>المبلغ (د.ع)</label>
            <input type="number" id="debt_amount" placeholder="المبلغ" min="1">

            <label>سبب الدين (اختياري)</label>
            <input type="text" id="debt_reason" placeholder="مثال: سلفة، قرض، إلخ">
        `;
    } else {
        title.textContent = "➕ دين علينا (لمجهز)";
        body.innerHTML = `
            <label>اختر مجهز موجود (أو اكتب اسم جديد)</label>
            <input type="text" id="debt_person" list="debtPersonsList" placeholder="اسم المجهز" autocomplete="off">
            <datalist id="debtPersonsList">
                ${data.suppliers.map(s => `<option value="${escapeHTML(s.name)}"></option>`).join("")}
            </datalist>
            ...
            <small>💵 المبلغ راح يدخل الخزنة تلقائياً (لأنه كاش دخل عندك)</small>
        `;
    }
...
function saveDebt() {
    ...
    const { type } = debtContext;

    const person = document.getElementById("debt_person").value.trim();
    const amount = Number(document.getElementById("debt_amount").value);
    const reason = document.getElementById("debt_reason").value.trim();

    if (!person || amount <= 0) {
        alert("تأكد من الاسم والمبلغ");
        return;
    }
    ...
    if (type === "customer") {
        // دين لنا على زبون
        let c = data.customers.find(
            x => x.name.toLowerCase() === person.toLowerCase()
        );
        if (!c) {
            c = { id: uid(), name: person, phone: "", debt: 0 };
            data.customers.push(c);
        }
        c.debt += amount;

        data.directDebts.push({
            id: uid(),
            type: "customer",
            person: person,
            amount: amount,
            reason: reason || "دين مباشر",
            date: today()
        });
        ...
        closeDebt();
        renderAll();
        alert(`✅ تمت إضافة دين ${money(amount)} د.ع على ${person}`);
    } else {
        // دين علينا لمجهز
        let s = data.suppliers.find(
            x => x.name.toLowerCase() === person.toLowerCase()
        ...
        if (!s) {
            s = { id: uid(), name: person, phone: "", debt: 0 };
            data.suppliers.push(s);
        }
        s.debt += amount;
        ...
        closeDebt();
        renderAll();
        alert(`✅ تمت إضافة دين ${money(amount)} د.ع لـ ${person}\n💵 دخل المبلغ للخزنة`);
    }
}
// ============================================================
// الحذف الذكي
...
function deleteSale(id) {
    ...
    if (product) restoreStock(product, sale.unit, sale.quantity);

    if (sale.debt > 0 && sale.customer && sale.customer !== "زبون نقدي") {
        const c = data.customers.find(
            x => x.name.toLowerCase() === sale.customer.toLowerCase()
        );
        if (c) c.debt = Math.max(0, c.debt - sale.debt);
    }
...
function deletePurchase(id) {
    ...
    if (purchase.debt > 0 && purchase.supplier && purchase.supplier !== "مجهز غير محدد") {
        ...
        if (s) s.debt = Math.max(0, s.debt - purchase.debt);
    }
...
function deleteCustomer(id) {
    ...
    const directDebts = data.directDebts.filter(
        d => d.type === "customer" && d.person === c.name
    );

    let msg = `⚠️ سيتم حذف الزبون: ${c.name}\n\n`;

    if (sales.length || payments.length || directDebts.length || c.debt > 0) {
        msg += `📋 العمليات المرتبطة:\n`;
        if (sales.length) msg += `• ${sales.length} مبيعات\n`;
        if (payments.length) msg += `• ${payments.length} تسديدات\n`;
        if (directDebts.length) msg += `• ${directDebts.length} ديون مباشرة\n`;
        if (c.debt > 0) msg += `• دين: ${money(c.debt)} د.ع\n`;
        msg += `\n`;

        if (sales.length) {
            const restore = confirm(
                msg + `هل تريد إرجاع المخزون للعمليات المرتبطة أيضاً؟\n\n(موافق = يرجع المخزون)`
            );
            if (restore) {
                sales.forEach(s => {
    ...
    data.directDebts = data.directDebts.filter(
        d => !(d.type === "customer" && d.person === c.name)
    );
...
function deleteSupplier(id) {
    ...
    const directDebts = data.directDebts.filter(
        d => d.type === "supplier" && d.person === s.name
    );

    let msg = `⚠️ سيتم حذف المجهز: ${s.name}\n\n`;

    if (purchases.length || payments.length || directDebts.length || s.debt > 0) {
        msg += `📋 العمليات المرتبطة:\n`;
        ...
        if (purchases.length) {
            const remove = confirm(
                msg + `هل تريد سحب المخزون للعمليات المرتبطة أيضاً؟\n\n(موافق = يسحب)`
            );
            if (remove) {
                purchases.forEach(p => {
...
function deleteProduct(id) {
    ...
    if (sales.length || purchases.length) {
        ...
        if (sales.length) {
            const restore = confirm(
                msg + `هل تريد إرجاع المخزون للمبيعات المرتبطة أيضاً؟\n\n(موافق = يرجع)`
            );
            if (restore) {
                sales.forEach(s => restoreStock(p, s.unit, s.quantity));
            }
        }
...
// ============================================================
// دمج المواد (عند التعديل إلى اسم موجود)
// ============================================================

function mergeProducts(sourceProduct, targetProduct) {
    // دمج حسب multi
    if (sourceProduct.multi && targetProduct.multi) {
        targetProduct.stockSmall = (Number(targetProduct.stockSmall) || 0) +
                                    (Number(sourceProduct.stockSmall) || 0);
    } else if (!sourceProduct.multi && !targetProduct.multi) {
        targetProduct.quantity = (Number(targetProduct.quantity) || 0) +
                                 (Number(sourceProduct.quantity) || 0);
    } else {
        // أنواع مختلفة — ما ندمج
        return false;
    }

    // نحدّث كل العمليات لتصير على الاسم الجديد
    const sourceName = sourceProduct.name;
    const targetName = targetProduct.name;
    const sourceId = sourceProduct.id;

    data.sales.forEach(s => {
        if (s.product.toLowerCase() === sourceName.toLowerCase()) {
            s.product = targetName;
        }
    });
    data.purchases.forEach(p => {
        if (p.product.toLowerCase() === sourceName.toLowerCase()) {
            p.product = targetName;
        }
    });

    // نحذف المادة المصدر
    data.products = data.products.filter(x => x.id !== sourceId);
    ...
    if (type === "product") {
        ...
        if (p.multi) {
            ...
            body.innerHTML = `
                ...
                <small>⚠️ إذا عدّلت الاسم لاسم مادة موجودة، راح تندمج المادتين</small>
                ...
                <small>(إجمالي الطبقات — مثلاً 12 = كارتونة واحدة)</small>
...
function saveEdit() {
    ...
    if (type === "customer") {
        ...
        if (oldName !== newName) {
            ...
            data.directDebts.forEach(d => {
                if (d.type === "customer" && d.person === oldName) {
                    d.person = newName;
                }
            });
        }

        // دمج مع زبون ثاني إذا نفس الاسم
        const other = data.customers.find(
        ...
        if (other) {
            ...
            closeEdit();
            renderAll();
            alert(`✅ تم دمج الزبون مع "${other.name}"`);
            return;
        }
    ...
    if (type === "supplier") {
        ...
        if (oldName !== newName) {
            ...
            data.directDebts.forEach(d => {
                if (d.type === "supplier" && d.person === oldName) {
                    d.person = newName;
                }
            });
        }

        const other = data.suppliers.find(
            x => x.id !== id && x.name.toLowerCase() === newName.toLowerCase()
        );
        if (other) {
            other.debt += newDebt;
            ...
            closeEdit();
            renderAll();
            alert(`✅ تم دمج المجهز مع "${other.name}"`);
            return;
        }
    ...
    if (type === "product") {
        ...
        if (oldName.toLowerCase() !== newName.toLowerCase()) {
            const other = data.products.find(
                x => x.id !== id && x.name.toLowerCase() === newName.toLowerCase()
            );

            if (other) {
                // موجودة مادة ثانية بنفس الاسم — ندمج
                const ok = mergeProducts(p, other);
                if (ok) {
                    p.name = other.name;
                    ...
                    closeEdit();
                    renderAll();
                    alert(`✅ تم دمج "${oldName}" مع "${newName}"\n\nالمخزون والأسعار صارت موحدة`);
                    return;
                } else {
                    alert("⚠️ ما يمكن الدمج — أنواع المواد مختلفة (وحدة واحدة vs متعددة)");
                    return;
                }
            } else {
                // ماكو دمج — بس نغير الاسم ونحدث العمليات
                p.name = newName;
...
// ============================================================
// حساب صافي الربح لفترة
// ============================================================

function calcNetForDate(dateStr) {
    const sales = data.sales.filter(s => s.date === dateStr)
        .reduce((sum, s) => sum + s.total, 0);
    const purchases = data.purchases.filter(p => p.date === dateStr)
...
function renderDashboard() {
    ...
    const yesterdayNet = calcNetForDate(y);
    const netDiff = todayNet - yesterdayNet;
    ...
    data.sales.forEach(s => {
        if (s.date >= monthStart && s.date <= monthEnd) daysInMonth.add(s.date);
    });
    data.purchases.forEach(p => {
        if (p.date >= monthStart && p.date <= monthEnd) daysInMonth.add(p.date);
    });
    data.expenses.forEach(e => {
        if (e.date >= monthStart && e.date <= monthEnd) daysInMonth.add(e.date);
    });

    let bestDayNet = 0;
    let bestDayDate = "—";
    daysInMonth.forEach(date => {
        const net = calcNetForDate(date);
        if (net > bestDayNet) {
            bestDayNet = net;
            bestDayDate = date;
        }
    ...
    setText("yesterdayNet", money(yesterdayNet));

    const diffEl = document.getElementById("netDiff");
    if (diffEl) {
        diffEl.textContent = (netDiff >= 0 ? "+" : "") + money(netDiff);
        diffEl.style.color = netDiff >= 0 ? "#16a34a" : "#dc2626";
    }

    setText("bestDayNet", money(bestDayNet));
    setText("bestDayDate", bestDayDate);
...
function buildCashMovements() {
    ...
    // ديون مباشرة علينا → دخلت كاش
    data.directDebts.forEach(d => {
        if (d.type === "supplier") {
            movements.push({
                id: "dd-" + d.id,
                type: "in",
                label: "دين علينا: " + (d.reason || "دين"),
                person: d.person,
                amount: Number(d.amount) || 0,
                date: d.date
            });
        }
        // دين لنا على زبون ما يدخل خزنة (ما استلمنا كاش)
    });
...
function showCustomerHistory(customerId) {
    ...
    const directDebts = data.directDebts.filter(d =>
        d.type === "customer" && d.person === c.name
    );
...
function showSupplierHistory(supplierId) {
    ...
    const directDebts = data.directDebts.filter(d =>
        d.type === "supplier" && d.person === s.name
    );
...
<div class="section">
    <h2>📒 ديون مباشرة (${data.directDebts.length})</h2>
    ${data.directDebts.length ? [...data.directDebts].reverse().map(d => `
        <div class="card">
            <div class="card-title">${d.type === "customer" ? "👤 لنا على: " : "🏪 علينا لـ: "}${escapeHTML(d.person)}</div>
            <div class="card-details">
                المبلغ: <strong>${money(d.amount)} د.ع</strong><br>
                السبب: ${escapeHTML(d.reason || "دين")}
                <br>📅 ${d.date}
            </div>
        </div>
    `).join("") : `<div class="empty">لا توجد ديون مباشرة</div>`}
</div>
    ...
    reader.onload = function (e) {
        try {
            ...
            data = {
                ...
                directDebts: importedData.directDebts || []
            };
